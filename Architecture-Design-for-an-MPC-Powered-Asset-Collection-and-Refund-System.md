...
### 3.2.6.1 Sign Service on AWS Nitro Enclave

为满足"云端参与方运行在可信执行环境且一次性加载数以万计 key share"的要求，Sign Service 采用 AWS Nitro Enclave 部署。该方案实现了硬件级别的安全隔离，确保密钥材料仅存在于 Enclave 内存中，Parent EC2 实例无法访问明文密钥。
...
flowchart TB
    subgraph Internet["外部网络"]
        Client["gRPC Client<br/>(Admin App)"]
    end

    subgraph AWS["AWS Cloud Services"]
        S3["S3 Bucket<br/>• age-private.key.enc<br/>• service_key_shares.json.age"]
        KMS["AWS KMS<br/>CMK for age key"]
        IMDS["EC2 IMDS<br/>Temporary Credentials"]
    end

    subgraph EC2["EC2 Parent Host"]
        RunScript["run-enclave.sh<br/>• 获取 IMDS 凭证<br/>• 下载 S3 密文<br/>• 启动 Enclave"]
        SocatIngress["socat Ingress<br/>TCP:50051 → vsock:50051"]
        SocatEgress["socat Egress<br/>vsock:8080 → TCP:8080"]
        VsockProxy["vsock-proxy<br/>vsock:5000 → KMS:443"]
        
        subgraph Enclave["Nitro Enclave (CID 16)"]
            Entrypoint["enclave-entrypoint.sh"]
            KmsTool["kmstool_enclave_cli<br/>(Attestation-based decrypt)"]
            AgeCLI["age CLI<br/>(Decrypt key shares)"]
            SignService["sign-service<br/>gRPC Server :50051"]
            KeyShares["Key Shares<br/>/dev/shm/<br/>(Memory only)"]
        end
    end

    subgraph Gateway["Sign Gateway"]
        SSE["SSE Endpoint :8080"]
        GRPC["gRPC Proxy"]
    end

    %% Client flow
    Client -->|"gRPC SignTx"| GRPC
    GRPC -->|"Proxy"| SocatIngress
    SocatIngress -->|"vsock:50051"| SignService
    SignService -->|"vsock:8080"| SocatEgress
    SocatEgress -->|"TCP:8080"| SSE

    %% Secret injection flow
    IMDS -->|"1. Fetch Credentials"| RunScript
    S3 -->|"2. Download Encrypted Files"| RunScript
    RunScript -->|"3. vsock:7100-7102<br/>Inject secrets"| Entrypoint

    %% KMS decryption flow
    Entrypoint -->|"4. Request decrypt"| KmsTool
    KmsTool -->|"vsock:5000<br/>+ Attestation"| VsockProxy
    VsockProxy -->|"TLS"| KMS
    KMS -->|"Verify PCR0"| KMS
    KMS -->|"PLAINTEXT"| VsockProxy
    VsockProxy -->|"Response"| KmsTool
    KmsTool -->|"age private key"| AgeCLI
    AgeCLI -->|"Decrypted shares"| KeyShares
    KeyShares --> SignService

    style Enclave fill:#ffe6e6,stroke:#cc0000
    style KMS fill:#fff3cd,stroke:#856404
...
| 端口 | 方向 | 用途 |
|------|------|------|
| 5000 | Enclave → Host → KMS | KMS vsock-proxy，用于 attestation-based 解密 |
| 7100 | Host → Enclave | AWS 临时凭证 (JSON) |
| 7101 | Host → Enclave | KMS 加密的 age 私钥密文 |
| 7102 | Host → Enclave | age 加密的 key shares 密文 |
| 8080 | Enclave → Host → Gateway | HTTP/SSE 出站通道 |
| 50051 | Host → Enclave | gRPC 入站通道 |

#### 拓扑与边界

- **EC2 Parent**（`c5.xlarge`、`m5.xlarge` 等支持 Nitro Enclave 的实例）：承载 Nitro Enclave，负责拉取密文、运行 vsock 代理、采集日志，**不持有明文密钥**。
- **Sign Gateway**：通过 VPC 私网访问 Parent 上的 gRPC 代理（TCP:50051），代理再经 vsock 转发到 Enclave 内部 `sign-service`。
- **VPC 安全组**：仅放行 Sign Gateway → Parent 的 TLS 流量；Enclave 无直接网络，所有出入口均经 vsock。
...
sequenceDiagram
    ...
    rect rgb(240, 255, 240)
        ...
        KMSTool->>KMSTool: 生成 attestation document (含 PCR0)
...
- **密文存储**：加密后的 age 私钥密文存储在 S3。
- **访问控制**：KMS Key Policy 配置 `kms:Decrypt` 条件，仅允许特定 PCR0 measurement 的 Enclave 解密：
...
- **安全边界**：Parent EC2 实例可以下载密文，但无法解密；只有通过 attestation 验证的 Enclave 内部进程才能获取明文。

#### 通信与代理

- **gRPC Ingress**：
    - Parent 启动 `socat TCP-LISTEN:50051,fork,reuseaddr VSOCK-CONNECT:16:50051`
    - 对外暴露 gRPC 端口，流量透传至 Enclave 内部 sign-service
    - Sign Gateway 与 Parent 建立连接，所有请求经 socat 转发

- **SSE/HTTP Egress**：
    - sign-service 需访问 Sign Gateway 的 SSE 端点
    - Parent 启动 `socat VSOCK-LISTEN:8080,fork,reuseaddr TCP-CONNECT:sign-gateway:8080`
    - 为 Enclave 提供出站 HTTP Client 通道

- **KMS Proxy**：
    - Parent 启动 `vsock-proxy 5000 kms.us-east-1.amazonaws.com 443`
    - 为 kmstool_enclave_cli 提供 KMS 访问通道
    - 自动处理 TLS 和 attestation document 传输
...
```bash
# 1. 初始化 submodules（包含 kmstool-enclave-cli）
git submodule update --init --recursive

# 2. 编译 sign-service 和 kmstool
...
# 输出: target/sign-service.enclave.eif
# 记录 PCR0 measurement 用于 KMS Key Policy
```
...
aws kms encrypt \
  ...
  --output text --query CiphertextBlob | base64 -d > age-private.key.enc

# 使用 age 加密 key shares
age -r $AGE_PUBLIC_KEY -o service_key_shares.json.age service_key_shares.json

# 上传到 S3
aws s3 cp age-private.key.enc s3://bucket/enclave/
aws s3 cp service_key_shares.json.age s3://bucket/enclave/
```
...
- **Attestation 记录**：每次启动记录 attestation document 与 share 的 SHA256 指纹，写入审计链路。
- **内存监控**：设定内存水位报警（如 70%）和 MPC 会话并发上限，防止 OOM。
- **健康检查**：Sign Gateway 定期发起 gRPC ping + SSE 订阅校验，超过阈值自动剔除该 Enclave 节点。
- **密钥生命周期**：
    - age 私钥解密后立即使用 `shred -u` 销毁
    - AWS 凭证使用后从内存清除
    - key shares 常驻 Enclave 内存，仅在实例重启时重新加载
- **日志审计**：Enclave 内部日志通过 `nitro-cli console` 流向 Parent，再转发到 CloudWatch；敏感数据默认 redact。

#### 详细文档

- [Sign-service-in-enclave.md](./Sign-service-in-enclave.md) - 完整的 Enclave 部署架构与安全设计
- [scripts/sign-service/README.md](./scripts/sign-service/README.md) - 构建和部署脚本详细说明
