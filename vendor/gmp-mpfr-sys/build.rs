...
fn get_actual_cross_target(cross_target: &str) -> &str {
    match cross_target {
        ...
        "aarch64-apple-ios" => "aarch64-apple-darwin11",
        "aarch64-apple-ios-sim" => "aarch64-apple-darwin11",
        "x86_64-apple-ios" => "x86_64-apple-darwin11",
        _ => cross_target,
    }
}
...
fn build_gmp(env: &Environment, lib: &Path, header: &Path) {
    ...
    conf.push_str(" --disable-shared --with-pic");
...
fn build_mpfr(env: &Environment, lib: &Path, header: &Path) {
    ...
    // 只在非 iOS 目标时清除环境变量
    if env.cross_target.as_ref().map_or(true, |t| !t.contains("apple-ios")) {
